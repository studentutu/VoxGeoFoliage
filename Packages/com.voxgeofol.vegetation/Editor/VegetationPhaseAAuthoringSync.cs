...
namespace VoxGeoFol.Features.Vegetation.Editor
{
    ...
    {
        private static readonly string[] PreferredDemoAssetPathFragments = { "Assets/Samples/", "Assets/Tree/" };
        private const string DemoAssemblyPrefabName = "tree_dense_branches";
        private const string DemoBlueprintName = "TreeBlueprint_branch_leaves_fullgeo";
        private const string DemoLodProfileName = "VegetationLODProfile";
        ...
        {
            // Range: runs only inside the Unity Editor with either imported package samples or the legacy repo demo assets present. Condition: rebuilds demo prototype bounds and blueprint branch placements from prefab authoring. Output: saved and validated Phase A demo assets.
            TreeBlueprintSO blueprint = LoadPreferredAssetByName<TreeBlueprintSO>(DemoBlueprintName);
            string demoAssetRoot = GetAssetFolderPath(AssetDatabase.GetAssetPath(blueprint));
            ...
            LODProfileSO lodProfile = LoadAssetByNameInFolder<LODProfileSO>(demoAssetRoot, DemoLodProfileName);
            string assemblyPrefabPath = FindPreferredAssetPath<GameObject>(DemoAssemblyPrefabName);
            ...
            RefreshBlueprintFromAssemblyAsset(blueprint, assemblyPrefabPath, branchPrototypes, lodProfile);
        ...
        private static T LoadPreferredAssetByName<T>(string assetFileNameWithoutExtension) where T : UnityEngine.Object
        {
            return LoadAssetAtPath<T>(FindPreferredAssetPath<T>(assetFileNameWithoutExtension));
        }

        private static T LoadAssetByNameInFolder<T>(string assetFolder, string assetFileNameWithoutExtension) where T : UnityEngine.Object
        {
            ...
            for (int i = 0; i < guids.Length; i++)
            {
                ...
                if (AssetPathMatchesFileName(assetPath, assetFileNameWithoutExtension))
                {
                    matchingPaths.Add(assetPath);
                }
            }

            return LoadAssetAtPath<T>(SelectPreferredPath(matchingPaths, assetFileNameWithoutExtension));
        }
        ...
        private static string FindPreferredAssetPath<T>(string assetFileNameWithoutExtension) where T : UnityEngine.Object
        {
            ...
            return SelectPreferredPath(matchingPaths, assetFileNameWithoutExtension);
        }

        private static string SelectPreferredPath(IReadOnlyList<string> matchingPaths, string assetFileNameWithoutExtension)
        {
            if (matchingPaths.Count == 0)
            {
                throw new InvalidOperationException(
                    $"Expected asset '{assetFileNameWithoutExtension}' was not found under Assets/. Import the package sample or restore the legacy demo assets.");
            }
            ...
            for (int preferredIndex = 0; preferredIndex < PreferredDemoAssetPathFragments.Length; preferredIndex++)
            {
                string preferredFragment = PreferredDemoAssetPathFragments[preferredIndex];
                for (int pathIndex = 0; pathIndex < matchingPaths.Count; pathIndex++)
                {
                    string candidatePath = matchingPaths[pathIndex];
                    if (candidatePath.IndexOf(preferredFragment, StringComparison.OrdinalIgnoreCase) >= 0)
                    {
            ...
            throw new InvalidOperationException(
                $"Asset '{assetFileNameWithoutExtension}' is ambiguous. Matches: {string.Join(", ", matchingPaths)}");
        }
        ...
        private static string GetAssetFolderPath(string assetPath)
        {
            ...
            {
                throw new InvalidOperationException($"Asset path '{assetPath}' does not resolve to a valid folder.");
            }

            return directoryPath.Replace('\\', '/');
        }

        private static bool AssetPathMatchesFileName(string assetPath, string assetFileNameWithoutExtension)
        {
            return string.Equals(
                Path.GetFileNameWithoutExtension(assetPath),
                assetFileNameWithoutExtension,
