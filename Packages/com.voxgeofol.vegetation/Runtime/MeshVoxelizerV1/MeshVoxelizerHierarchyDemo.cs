...
{
    /// <summary>
    /// Sample component that visualizes a MeshVoxelizer-based hierarchy with 16/12/8 voxel levels per node.
    /// </summary>
    public sealed class MeshVoxelizerHierarchyDemo : MonoBehaviour
    {
        [Tooltip("Highest detail shell resolution. Defaults to the verified 16 subdivision level.")]
        [SerializeField] private int voxelResolutionL0 = 16;
        [Tooltip("Mid detail shell resolution. Defaults to the verified 12 subdivision level.")]
        [SerializeField] private int voxelResolutionL1 = 12;
        [Tooltip("Lowest detail shell resolution. Defaults to the verified 8 subdivision level.")]
        [SerializeField] private int voxelResolutionL2 = 8;
        [SerializeField] private int maxDepth = 2;
        [SerializeField] private int minimumSurfaceVoxelCountToSplit = 4;
        ...
        private void CreateNodeRecursive(Transform parentTransform, MeshVoxelizerHierarchyNode[] nodes, int nodeIndex)
        {
            ...
            {
                CreateMeshChild(nodeObject.transform, $"ShellL0_{voxelResolutionL0}", node.ShellL0Mesh);
            }

            if (showShellL1 && node.ShellL1Mesh != null)
            {
                CreateMeshChild(nodeObject.transform, $"ShellL1_{voxelResolutionL1}", node.ShellL1Mesh);
            }

            if (showShellL2 && node.ShellL2Mesh != null)
            {
                CreateMeshChild(nodeObject.transform, $"ShellL2_{voxelResolutionL2}", node.ShellL2Mesh);
            }
