...
- Development: [Agentic Workflow](#agentic-workflow), [Repo Requirements](#repo-requirements)
...
4. Build entry points are defined in [.vscode/tasks.json](.vscode/tasks.json).
5. Use kiss-unity-mcp for CI/Compilation/Verification/Shaders-compilation/Tests.

### Local verification tooling

kiss-unity-mcp `0.6.0` is installed as an embedded editor package with standalone Bash tools under `.kissunitymcp`.

1. Open [.vscode/kissunitymcp.code-workspace](.vscode/kissunitymcp.code-workspace) or use the root [.vscode/tasks.json](.vscode/tasks.json).
2. Run `kiss-unity-mcp: Check tool paths`. Optional tool overrides belong in [.kissunitymcp/tools.env](.kissunitymcp/tools.env); defaults discover Unity and Rider automatically.
3. Close interactive Unity, then run `kiss-unity-mcp: unity-import-long-compile` to import assets, compile C#, regenerate the solution, and establish the fast-build snapshot.
4. Use `Fast MSBuild` for edits to existing C# files after a successful import. Use full import for asset, package, or project-membership changes. Separate tasks run EditMode tests, parse saved results, and compile shaders.

Tasks invoke Git Bash through Git. The manual dispatcher is `.kissunitymcp/scripts/unity.sh`; verification artifacts go to ignored `Logs/kissunitymcp`.

## Repo Requirements

1. Unity Hub with the exact editor version in [ProjectSettings/ProjectVersion.txt](ProjectSettings/ProjectVersion.txt), currently `6000.3.15f1`.
2. Git and Git Bash.
