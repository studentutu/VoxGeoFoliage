...
- `Packages/com.voxgeofol.vegetation/Runtime`
  ...
  - `Runtime/Rendering/` holds the production runtime path: `VegetationRenderWorld`, compiled provider registration through `VegetationRuntimeContainer` and `Vegetation.SubScene`, scheduled page/cell culling, packet selection, budgeting, compaction, shader wind binding, URP RenderGraph preparation contract, and grouped-indirect depth/color/shadow passes. The retired tree-first runtime family and BRG runtime backend were deleted.
  ...
  - `Runtime/Shaders/VegetationCanopyLit.shader`, `VegetationTrunkLit.shader`, `VegetationFarMeshLit.shader`, `VegetationDepthOnly.shader`, and `VegetationIndirectCommon.hlsl` are the runtime shader suite. The active renderer uses grouped-indirect `_VegetationInstanceData` and `_VegetationInstanceDataBaseOffset`; legacy DOTS shader variants still exist in shader code but are not an active C# runtime path. Shader wind uses compiled tree phase/anchor data plus global wind and leaf-flutter settings. Shadow support is main-light directional only through `VegetationShadowMode.Off` / `CheapTree`.
...
- runtime rendering authority is `VegetationRenderWorld` through `Runtime/Rendering`; it consumes compiled `FoliagePageAsset` packets from classic-scene and SubScene providers. The only C# runtime backend is URP RenderGraph grouped-indirect submission with an explicit compute preparation contract. Selection is page/cell based, nearest-to-farthest, budgeted globally, HLOD-backed, and free of synchronous GPU readbacks. The current vertical slice schedules cull/select/budget/compaction/args generation as a job, records only already-completed preparation frames into RenderGraph, and lets the RenderGraph compute pass upload those completed frames without `JobHandle.Complete()` in graph execution. The next optimization target is replacing that scheduled job with GPU compute kernels after dense-scene validation.
...
- Runtime has one primary C# backend: URP RenderGraph grouped-indirect submission. Missing compiled page assets, incompatible package shaders, missing compute contract resources, or unsupported draw setup must log and skip/fault-disable instead of throwing inside render callbacks.
- RenderGraph color/depth raster passes consume imported instance/args buffers plus the compute contract buffer and must avoid global-state mutation. RenderGraph compute/raster execution must never complete scheduled preparation jobs; graph execution may only upload and draw frames that were completed before recording. This is still an intermediate slice. The target cutout in `DetailedDocs/VegetationRenderGraphGpuDrivenPlan.md` moves clear, visibility, admission/budgeting, compaction, and indirect-args writes into RenderGraph compute passes, then raster draws all group entries with zero-count indirect args for inactive entries. The shadow pass still mutates global shadow view/projection/depth-bias state to append into URP's main shadow atlas; remove that in a later shadow-specific hardening pass if URP exposes a cleaner contract.
...
- Do not reintroduce `BatchRendererGroup`/BRG as a second production backend. The package runtime must have one explicit RenderGraph path, not per-API routing or fallback backend logic.
- Vegetation shadow injection must be a RenderGraph raster pass that binds URP `mainShadowsTexture` as a `ReadWrite` depth attachment. Do not use unsafe passes or manual `SetRenderTarget` for the main shadow atlas.
