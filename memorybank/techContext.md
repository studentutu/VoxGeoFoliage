...
- `Packages/com.voxgeofol.vegetation/Runtime`
  ...
  - `Runtime/Rendering/` now holds the Phase E runtime path: Phase D contracts/registration, the CPU reference decode fallback, the GPU-resident compute decode/emission path, the optional GPU decision readback bridge, the indirect renderer submission path, the URP renderer feature/pass integration, and the Scene-view debug demos for both classification and submitted draw batches
  ...
  - `Runtime/Rendering/Debug/DebugVegetationDemo.cs` is the Phase E Scene-view gizmo verifier for uploaded indirect batches and decoded visible instances driven by the runtime manager; in GPU-resident mode it can rely on conservative batch bounds when exact CPU-side counts are not available
  ...
  - runtime rendering authority is `Graphics.RenderMeshIndirect` through `Runtime/Rendering`; current shipped Phase E implementation classifies and emits the visible frontier on GPU through `VegetationGpuDecisionPipeline`, binds shared GPU-written instance/args buffers through `VegetationIndirectRenderer`, renders through `VegetationRendererFeature`, and also exposes CPU reference plus an optional non-blocking GPU decision readback bridge for fallback and validation
