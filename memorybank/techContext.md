...
- runtime rendering authority is `Graphics.RenderMeshIndirect` through `Runtime/Rendering`; current shipped implementation accepts one tree tier per visible tree on GPU through `VegetationGpuDecisionPipeline`, keeps admission linear via priority-ring ordering instead of O(N^2) rescans, splits per-view budgets into visible-instance memory, expanded-branch queue growth, approximate accepted-content work, and registered draw-slot cap, fault-disables camera/color preparation explicitly if visible non-far `TreeL3` baseline cannot fit the color approximate work budget, expands branch work only for promoted trees, dispatches branch count/emit from GPU-built indirect args using actual generated work count, captures actual usage telemetry and active-slot subsets from latest completed async GPU readbacks, hands camera/shadow submission off through explicit `VegetationIndirectRenderer.PreparedViewHandle` bindings instead of one mutable renderer-global frame, preserves signed `BaseVertexLocation` in indirect args, and keeps the production prepare/render path free of synchronous GPU readbacks
...
- current runtime-review diagnostics also report latest async actual visible-instance count, actual generated branch-work count, and budget-cap-hit flags, while the active-slot submission surface uses the latest completed emitted-slot readback with registered-slot fallback during warm-up
...
- `Packages/com.voxgeofol.vegetation/Editor`
  - Milestone1 Phase B canopy/wood/trunk/impostor bake tooling, Phase C preview/inspector/window tooling, the Milestone1 Phase C.5 gate summary in `VegetationTreeAuthoringEditorPanel`, and the editor-only simplification/fallback helpers used by canopy, wood, trunk, and impostor generation
...
- Editor-baked voxel artifacts must be clipped back to authoritative source bounds; this applies to canopy tier meshes, generated branch wood, simplified trunk meshes, and impostor meshes. Temporary voxel hierarchies are bake-internal only and must not persist on `BranchPrototypeSO`.
- Current vegetation runtime authority is the urgent tree-first path: `Impostor` far only, `TreeL3` mandatory non-far floor, and promoted branch-expanded `L2/L1/L0` built from reusable blueprint placements plus compact branch prototype tier meshes. Pre-populated `SceneBranches[]` and runtime temporary-hierarchy ownership are removed from the production path.
...
- URP feature hard constraint: never share one mutable prepared resident vegetation frame across camera and shadow or explicit-frustum consumers in the same render cycle. Camera and shadow preparation need separate pipeline ownership or equivalent buffering.
