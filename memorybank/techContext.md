...
- `Packages/com.voxgeofol.vegetation/Runtime`
  ...
  - `Runtime/Rendering/` holds the production runtime path: `VegetationRenderWorld`, compiled provider registration through `VegetationRuntimeContainer` and `Vegetation.SubScene`, global page/cell culling, packet selection, grouped indirect submission, shader wind binding, and the URP renderer feature/pass integration. The retired tree-first runtime family was deleted.
  ...
  - `Runtime/Shaders/VegetationCanopyLit.shader`, `VegetationTrunkLit.shader`, `VegetationFarMeshLit.shader`, `VegetationDepthOnly.shader`, and `VegetationIndirectCommon.hlsl` are the runtime shader suite used by indirect vegetation submission; the indirect instance payload now includes compiled wind metadata, and the render world binds global wind constants, runtime leaf-flutter constants, plus `_VegetationInstanceDataBaseOffset` per grouped draw. Shader trunk-following displacement advances from one compiled tree phase plus global time; branch/canopy packets keep full trunk-following bend weight. Canopy shaders use compiled branch flutter metadata only as small local vertex flutter over the trunk-following position, never as additional shared wind-direction displacement; local flutter amplitude, speed, spatial scale, and secondary harmonic strength come from `VegetationFoliageFeatureSettings`. `_VegetationInstanceData` is declared only when both Unity procedural instancing and `_VOXGEOFOL_INDIRECT_RENDERING` are enabled, so regular MeshRenderer/editor-preview draws use object matrices and do not trip DirectX/Vulkan unbound-buffer validation. Shadow support is limited to main-light directional shadows through `VegetationShadowMode.Off` / `CheapTree`.
...
- runtime rendering authority is `VegetationRenderWorld` through `Runtime/Rendering`; the current shipped implementation consumes compiled `FoliagePageAsset` packets from classic-scene and SubScene providers, uses page/cell `CullingGroup` broad phase for camera passes, uses one batched page/cell cascade-frustum pass for main-light shadows, request-loads cell-level near-detail residency under resident/upload byte budgets, selects visible cells nearest-to-farthest, cascades from the best distance tier to cheaper near-detail tiers under global work/instance budgets, always falls back to HLOD impostor packets for visible trees when detail budgets fail, uploads grouped instance data, emits indirect args per compiled `FoliageAssetGroup`, binds shader wind constants and explicit grouped instance-buffer base offsets, preserves signed `BaseVertexLocation` in indirect args, writes zero indirect `startInstance`, and keeps the prepare/render path free of synchronous GPU readbacks
...
- Runtime is indirect/GPU-buffer based only. Missing compiled page assets, incompatible package shaders, or unsupported indirect drawing must log and skip/fault-disable instead of throwing inside render-graph execution.
- Render-graph raster passes that submit vegetation indirect draws must opt into global-state mutation where needed because the renderer enables `_VOXGEOFOL_INDIRECT_RENDERING` around grouped indirect draws and binds shared instance buffers and wind constants through per-draw property blocks instead of per-slot material copies.
...
- DirectX also requires vegetation shadow injection to be a render-graph raster pass that binds URP `mainShadowsTexture` as a `ReadWrite` depth attachment. Do not use unsafe passes or manual `SetRenderTarget` for the main shadow atlas.
