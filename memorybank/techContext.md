...
- `Packages/com.voxgeofol.vegetation/Runtime`
  ...
  - `Runtime/Shaders/VegetationCanopyLit.shader`, `VegetationTrunkLit.shader`, `VegetationFarMeshLit.shader`, `VegetationDepthOnly.shader`, and `VegetationIndirectCommon.hlsl` are runtime shader suite used by indirect vegetation submission; the visible package shaders now carry main-light shadow attenuation and `ShadowCaster` passes, but the production indirect runtime still only submits depth/color passes and does not yet inject vegetation into URP shadow atlases
