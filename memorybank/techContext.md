...
- `Packages/com.voxgeofol.vegetation/Runtime`
  ...
  - `Runtime/Shaders/VegetationClassify.compute` is compute entry point used by GPU-resident classification and indirect emission
  ...
  - runtime rendering authority is `Graphics.RenderMeshIndirect` through `Runtime/Rendering`; current shipped implementation classifies and emits the visible frontier on GPU through `VegetationGpuDecisionPipeline`, binds shared GPU-written instance/args buffers through `VegetationIndirectRenderer`, and renders through `VegetationRendererFeature`
...
- Current vegetation runtime authority docs target indirect rendering plus runtime `L0/L1/L2/L3 + Impostor`, not BRG or one tree-wide runtime tier; the shipped runtime path  has working GPU-resident decode plus indirect rendering/depth/color submission, and old CPU/decode helpers are removed from `Runtime/Rendering`.
