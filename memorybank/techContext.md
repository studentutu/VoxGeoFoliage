...
- `Packages/com.voxgeofol.vegetation/Runtime`
  ...
  - `Runtime/Rendering/Debug/DebugVegetationDemo.cs` is the Phase E Scene-view gizmo verifier for uploaded indirect batches and decoded visible instances driven by the runtime manager
  ...
  - runtime rendering authority is `Graphics.RenderMeshIndirect` through `Runtime/Rendering`; current shipped Phase E implementation uploads per-slot instance payloads/args/bounds through `VegetationIndirectRenderer`, renders through `VegetationRendererFeature`, defaults to CPU-prepared visible output, and also exposes an optional non-blocking GPU decision readback bridge for delayed decode validation
