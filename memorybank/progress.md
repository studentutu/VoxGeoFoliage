...
- the render-thread `ExecuteRenderGraph` stall caused by completing the scheduled preparation job inside the compute contract has been cut out. Shadow frames now write per-cascade compacted instance spans/args and `CheapTree` rejects `SameAsColor` packets so cascades do not redraw whole group-wide ranges or replay near-detail color geometry. The current blocker is validating completed-frame latency, dense-scene slot pressure, and GPU shadow/color draw volume, then replacing the scheduled job with GPU compute kernels if profiling still shows preparation cost or stale-frame artifacts.
