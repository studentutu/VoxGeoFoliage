...
- [x]`C5-02` Re-bake `Packages/com.voxgeofol.vegetation/Samples~/Vegetation Demo/VoxFoliage/TreeBlueprint_branch_leaves_fullgeo.asset` and confirm assigned `trunkL3Mesh`.
- [x] `C5-03` Re-bake `Assets/Tree/VoxFoliage/TreeBlueprint_branch_leaves_fullgeo.asset` and confirm assigned `trunkL3Mesh`.
- [x] `C5-04` Run manual in-Editor visual verification on both exact tree-blueprint assets for `L0`, `L1`, `L2`, `L3`, shell-only `L1`, shell-only `L2`, shell-only `L3`, and `Impostor`.
- [x] `C5-05` Compare baked `L1/L2` compact hierarchies against `Assets/Tree/Raw/branch_leaves_quadcards.obj` as the current silhouette reference input.
- [x] `C5-06` Tune `ShellBakeSettings` and `ImpostorBakeSettings` on both exact tree-blueprint assets until validation passes without `MeshLodUtility` fallback.
