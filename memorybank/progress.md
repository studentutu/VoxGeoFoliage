...
- `2026-04-11`: Reworked both readmes into a clean split: `Packages/com.voxgeofol.vegetation/README.md` is now the authoritative feature contract, while root `Readme.md` is the repo entry point with repo docs, scripts, and agentic workflow guidance layered on top.
- `2026-04-11`: Package README now defines draw slots as the real draw-call unit in the runtime and adds concrete grass-like authoring strategies, including the tradeoff between one clump blueprint with many branches versus many small authorings with one branch each.
- `2026-04-11`: Reorganized both readmes for newcomer navigation: both now lead with a summary and highlights, setup/use-case/limitation sections are easier to scan, ordered lists are used where sequence matters, and the package README pipeline is documented in compact `A -> B -> C` form.
