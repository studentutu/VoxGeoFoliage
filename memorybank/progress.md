...
- Restored per-frame CPU active-slot filtering in `BindGpuResidentFrame()` as a temporary correctness/stability guard so the renderer submits only non-zero emitted slots and skips all-zero frames instead of issuing zero-instance indirect draws that can remove the D3D12 device.
...
- Reproduce the dense-forest crash with `VegetationFoliageFeatureSettings.EnableDiagnostics` enabled and capture the new per-container totals (`totalComputeBufferBytes`, `totalGraphicsBufferBytes`, `totalGpuBufferBytes`, `registeredDrawSlots`, `runtimeMaterialCopies`) before the failure.
- Re-run the isolated impostor-only repro and capture the new `VegetationIndirectRenderer` per-slot metadata (`mesh`, `indexCount`, `startIndex`, `baseVertex`, `shader`) for the single submitted `FarMesh` slot to confirm whether the signed base-vertex fix clears the D3D12 device removal.
- Re-run the isolated impostor-only repro and capture the full submitted-slot metadata (`mesh`, `vertexCount`, `subMeshes`, `topology`, `indexFormat`, `indexCount`, `startIndex`, `baseVertex`, `shader`, `sourceMaterial`) for the single submitted `FarMesh` slot so we can separate malformed generated mesh/material state from a deeper D3D12 or render-graph issue.
- Re-run the isolated impostor-only repro and confirm `runtimeMaterialCopies=0`, then capture the full submitted-slot metadata (`mesh`, `vertexCount`, `subMeshes`, `topology`, `indexFormat`, `indexCount`, `startIndex`, `baseVertex`, `shader`, `sourceMaterial`) for the single submitted `FarMesh` slot so we can separate malformed generated mesh/material state from a deeper D3D12 or render-graph issue.
