...
3. Recompile Playground and PineForest/Rootplacement foliage assets and compare against master captures with `CheapTree`, 4 cascades, scene rendering disabled, and SceneView cameras disabled.
4. Add procedural placement output as compiled page providers.
5. Add externalized async near-detail payload providers when pages move out of direct ScriptableObject references.
6. Run production verification without HZB across 100k loaded instances, 1M streamed instances, mobile profile, VR stereo profile, shadows, and wind.
