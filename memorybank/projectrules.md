...
21. The bundled package shaders must carry the package indirect-instance contract for forward, depth, and shadow-caster usage. Current shipped runtime support includes main-light shadow attenuation, `ShadowCaster` passes, and a real URP main-light shadow-atlas append pass. Current shadow limits are strict: main-light directional shadows only, cascade-specific resident frames derived from the camera-visible vegetation set, default `TreeL3`/impostor shadow casters with expanded branch shadow promotion disabled unless explicitly enabled, and no additional-light shadow atlas integration yet.
22. Trunk stays full for runtime `L0` and `L1`; runtime `L2` uses simplified `trunkL3Mesh`; non-promoted visible trees inside the non-far bands use `treeL3Mesh`; far trees use `impostorMesh` only.
23. Generated meshes that miss budgets still persist and remain wired, but validation marks the owning asset invalid.
24. `VegetationRuntimeContainer` registration is a frozen runtime snapshot. After container enable, transform edits on registered `VegetationTreeAuthoring` objects and other registration-affecting scene or authoring changes (position/rotation/scale) are not live-synced; explicit `RefreshRuntimeRegistration()` is required.
25. Each `VegetationRuntimeContainer` owns only active `VegetationTreeAuthoring` references from its serialized list, and every referenced authoring must stay inside that container hierarchy. Nested child containers claim their own descendants when editor fill tooling rebuilds the list, so streaming/addressable chunks must be structured by container hierarchy instead of scene-global discovery.
26. `VegetationRuntimeContainer` has no CPU fallback. Missing compute support, missing `VegetationClassify.compute`, or shader-import failures are hard blockers that must fail explicitly through logged skip/fault-disable behavior in runtime rendering, not through render-loop throws.
27. `AuthoringContainerRuntime` is the single authoritative runtime owner in all modes. `VegetationRuntimeContainer` is only the classic-scene lifecycle provider, and closed `SubScene` support must flow through `SubSceneAuthoring` plus the separate `Vegetation.SubScene` asmdef.
28. Shared registration input is `VegetationTreeAuthoringRuntime`, not live `VegetationTreeAuthoring`. Runtime registration and renderer discovery must not regain direct `MonoBehaviour` dependencies.
29. Closed `SubScene` support must not pull DOTS into the main `Vegetation` asmdef. Bakers, baked components, and bootstrap systems live in `Vegetation.SubScene`; `AuthoringContainerRuntime`, `VegetationTreeAuthoringRuntime`, and renderer integration stay in the main package assembly.
30. Runtime registration must not pre-populate `SceneBranches[]`, duplicate per-scene shell-node world bounds, or upload prototype shell-node buffers for the production path. Persisted shell hierarchies live once per prototype for editor-only shell preview; runtime branch bounds are derived on demand from tree transforms plus reusable blueprint placements.
31. Runtime visible-instance memory must stay hard-bounded.
32. Container capacity is not a global scene coordinator. Multiple active runtime owners each own independent packed instance buffers, indirect args, and GPU pipeline state. Splitting a forest across containers is allowed and is the intended chunking workaround when one container would overflow, but total memory and visible capacity then scale with the number of visible containers.
33. All runtime-review telemetry must stay behind `VegetationFoliageFeatureSettings.EnableDiagnostics`.
...
35. GPU classify/count/emit code must guard every runtime-derived buffer index before UAV writes or indirect-args writes. Invalid draw-slot, blueprint, placement, prototype, or cell indices must drop work rather than issuing undefined GPU memory access that can remove the D3D12 device.
36. Current runtime material compatibility is still a known limitation, not a stable public extension point: the package-compatible path now uses the provided materials directly, but those materials must implement the vegetation indirect shader contract for forward/depth/shadow submission. `VegetationIndirectMaterialFactory` is no longer a runtime authority and only remains as a no-op placeholder until the next full Unity solution regeneration. Milestone 2 must replace the current package-only assumptions with an explicit compatible-material contract or binding path for project-local custom materials.
37. Hierarchical wind is not shipped yet. When it lands, it must stay tier-consistent across `L0/L1/L2/L3/Impostor` and be part of the same public compatible-material contract instead of a package-only hidden shader path.
...
- `VegetationRendererFeature` - URP integration: stores the shared `VegetationClassify.compute` reference and diagnostics toggle in `VegetationFoliageFeatureSettings`, schedules the indirect vegetation main-light shadow/depth/color passes, appends camera-visible vegetation casters into the URP main-light shadow atlas through a render-graph unsafe pass, defaults the shadow path to `TreeL3`/impostor casters unless expanded shadow promotion is explicitly enabled, and must fault-disable instead of throwing from URP callbacks; it does not own classification or decode rules
- `VegetationIndirectRenderer` - indirect rendering authority: draw-slot submission metadata, shared GPU-resident packed instance/args/slot-start buffers, explicit per-draw global slot/buffer binding, non-zero-slot submission after bind via the current synchronous safety readback, conservative uploaded-batch snapshots for diagnostics, per-slot draw metadata logging for crash isolation, and `RenderMeshIndirect` submission. The long-term target is still a non-stalling submission filter.
