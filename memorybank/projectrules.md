...
10. Runtime broad phase is page/cell only. Use BRG `BatchCullingContext` split planes on supported BRG APIs and `CullingGroup` only for Direct3D12/fallback camera visibility. Do not feed per-tree spheres to runtime broad phase.
...
16. `BatchRendererGroup` is the production backend on supported raw-buffer APIs. Submission is one BRG batch and one batch-owned raw `GraphicsBuffer` per compiled `FoliageAssetGroup`, with built-in matrices plus `_VegetationPackedLeafTint` and `_VegetationWind`.
17. BRG callbacks must use immutable snapshots, callback-local scratch, validated group-local visible instance ranges, cleared `BatchCullingOutput.customCullingResult[0]`, compacted visible instances, and deferred retired-resource disposal.
18. Register BRG `Light` culling only for `VegetationShadowMode.CheapTree` on stable graphics APIs. Do not register custom vegetation BRG on Unity `6000.3.15f1`; use grouped-indirect RenderGraph there.
19. The grouped-indirect RenderGraph path must use `GraphicsBuffer.Target.IndirectArguments` only, keep indirect `startInstance` zero, and index `_VegetationInstanceData` through `_VegetationInstanceDataBaseOffset`.
20. Runtime shadow support is main-light directional only through `VegetationShadowMode.Off` and `VegetationShadowMode.CheapTree`. No additional-light vegetation shadow atlas exists yet.
21. `Off` skips vegetation shadow-caster submission. `CheapTree` submits compiled shadow packets under `ShadowWorkBudget`; shadow selection must be subordinate to compiled packet metadata and must not invent richer or enlarged caster geometry.
22. Shadow budget pressure follows the same degradation policy as color: near detail -> cell HLOD -> page HLOD -> culled. Do not create independent high-detail shadow work after color has degraded.
23. BRG shadow caster draws are renderer-owned and must not manually bind the shadow atlas. Fallback grouped-indirect shadow injection must append into URP's main shadow atlas through a RenderGraph raster depth attachment with `AccessFlags.ReadWrite`.
24. Fallback shadow draws must bind GPU-adjusted cascade view/projection globals per cascade and restore camera globals afterward. Do not rely on URP's post-shadow camera state.
25. Per-instance color variation uses one packed uint path. In BRG it is `_VegetationPackedLeafTint`; in fallback indirect draws it is read from `_VegetationInstanceData`. Do not use `MaterialPropertyBlock` for per-instance variation.
26. Runtime material compatibility is explicit: forward/depth/shadow passes must include BRG/DOTS metadata for matrices, `_VegetationPackedLeafTint`, `_VegetationWind`, global wind constants, fallback indirect instance layout, and `_VegetationInstanceDataBaseOffset`. `_VegetationInstanceData` must exist only for procedural non-DOTS variants.
27. Canopy and HLOD shaders are minimal vertex-lit. Trunk shader uses albedo texture but no normal map. Runtime must not manufacture per-slot material copies for compatibility.
28. No Unity `LODGroup`; LOD selection is owned by authored distance bands plus compiled packet selection. HZB remains a later accelerator only after packet renderer production validation.
29. Shader wind is runtime-bound global constants plus compiled `FoliageWindMetadata`. Do not rebuild wind metadata per frame. Use owning tree bounds for wind phase/anchor across trunk, branch, canopy, and HLOD packets; packet bounds remain for culling only.
30. Runtime rendering resource owners must be exception-safe on partial construction. Diagnostics must stay behind `EnableDiagnostics`, avoid steady-state allocations, and preserve profiler markers for graph refresh, broad phase, selection, upload, args, buffer upload, render-graph record, execute, draw submit, and shadow restore.
31. Shader, BRG, and indirect draw code must guard runtime-derived buffer indices before buffer reads, BRG visible-instance writes, or indirect-args writes. Invalid packet, asset-group, pass, or instance indices must drop work.

## Wiring Hubs

- `VegetationRenderWorld` - production runtime hub: owns compiled providers, BRG batches and batch-owned instance buffers on supported raw-buffer APIs, Direct3D12/fallback grouped-indirect buffers, page/cell broad phase, packet selection, color/shadow budgets, near-detail resident/upload budgets, shader wind constants, diagnostics counters, and resource reset.
