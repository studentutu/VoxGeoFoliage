...
31. Current runtime material resolution is a known limitation, not a stable public extension point: `VegetationIndirectMaterialFactory` still rewrites draw slots to package shader names. Milestone 2 must replace this with an explicit compatible-material contract or binding path for project-local custom materials.
32. Hierarchical wind is not shipped yet. When it lands, it must stay tier-consistent across `L0/L1/L2/L3/Impostor` and be part of the same public compatible-material contract instead of a package-only hidden shader path.
