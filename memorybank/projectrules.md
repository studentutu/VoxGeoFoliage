...
15. Canopy/impostor shaders are **minimal vertex-lit**: no albedo texture, no normal map, no emission, no specular — color from vertex data, normals from vertex data, lighting via Lambert + URP main light. Trunk shader uses albedo texture but no normal map.
