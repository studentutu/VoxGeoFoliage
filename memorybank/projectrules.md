...
9. GPU runtime work is staged: GPU cell visibility, tree classification, branch tier selection, hierarchy survival decisions, GPU-resident frontier emission, and indirect draw submission. Current shipped Phase E runtime path is `GpuResident`; CPU reference remains fallback/parity only, while the optional GPU readback bridge stays non-blocking and delayed for validation.
...
- `VegetationRuntimeManager` - runtime orchestrator: gathers `VegetationTreeAuthoring` instances, freezes the runtime registry, maintains runtime tree indices, prepares one per-camera visible frame snapshot, owns the GPU-resident compute path plus the optional GPU-decision readback bridge, and binds the latest indirect draw resources into `VegetationIndirectRenderer`; current contract is explicit snapshot registration, not live transform or content sync
...
- `VegetationGpuDecisionPipeline` - Phase D/Phase E GPU authority: uploads the frozen registry into compute buffers, mirrors the CPU decision contracts for parity when needed, and in the main runtime path emits GPU-resident indirect instance payloads plus indirect args directly into shared draw buffers; environments that import the shader without kernels must fail explicitly instead of silently faking GPU success
...
- `DebugVegetationDemo` - editor-side Phase E verification authority: drives `VegetationRuntimeManager` frame preparation for a preview camera and draws the decoded visible instances when CPU/debug data exists plus the uploaded indirect batch bounds/counts; GPU-resident mode exposes conservative batch bounds and may not have exact CPU-visible instance counts
