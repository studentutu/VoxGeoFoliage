...
- `MeshVoxelizerHierarchyBuilder` / `MeshVoxelizerHierarchyDemo` - shared Phase B hierarchy authority and manual inspection utility: voxelizes a readable mesh at `16/12/8`, splits L0 surface voxels into octant nodes, and emits one mesh triplet per node for the current canopy bake path
- `CPUVoxelizer` / `CpuVoxelSurfaceMeshBuilder` - shared CPU voxel backend authority: builds indexed voxel volumes and surface-only meshes, currently used by production impostor baking and kept as the reference backend for the future canopy rewrite
