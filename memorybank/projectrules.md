...
9. GPU runtime work is staged: GPU cell visibility, tree visibility classification, mandatory `TreeL3` acceptance, nearest-first promotion into `L2/L1/L0`, promoted-tree-only compact branch work generation, indirect count/pack, and final submission. Current shipped renderer submits the registered draw-slot surface after bind because synchronous CPU non-zero-slot compaction stalled the frame; exact non-zero slot counts remain diagnostics-only telemetry until a non-stalling path exists. Current runtime path is `GpuResident` only. Legacy CPU/decode runtime rendering was removed from `Runtime/Rendering` and must not be reintroduced into the production container path.
...
21. The bundled package shaders must carry the package indirect-instance contract for forward, depth, and shadow-caster usage. Current shipped shader support includes main-light shadow attenuation and `ShadowCaster` passes, but the production indirect runtime still does not submit vegetation into URP shadow atlases.
...
26. `VegetationRuntimeContainer` has no CPU fallback. Missing compute support, missing `VegetationClassify.compute`, or shader-import failures are hard blockers that must fail explicitly.
...
35. Current runtime material resolution is a known limitation, not a stable public extension point: `VegetationIndirectMaterialFactory` still rewrites draw slots to package shader names. Milestone 2 must replace this with an explicit compatible-material contract or binding path for project-local custom materials.
36. Hierarchical wind is not shipped yet. When it lands, it must stay tier-consistent across `L0/L1/L2/L3/Impostor` and be part of the same public compatible-material contract instead of a package-only hidden shader path.
...
- `VegetationIndirectRenderer` - indirect rendering authority: runtime material copies, shared GPU-resident packed instance/args/slot-start buffers, registered-slot submission after bind, conservative uploaded-batch snapshots for diagnostics, and `RenderMeshIndirect` submission. Non-zero-slot compaction is deferred until it can be done without a bind-path stall.
