...
16. Spatial partitioning via uniform grid; GPU is the primary visibility path. CPU frustum test plus optional `CullingGroup` is temporary fallback only when the GPU path is unavailable or explicitly disabled. Dense-forest urgent work must follow `DetailedDocs/urgentRedesign.md`: accepted-content prioritization first, occlusion later only if telemetry proves it is still worth the complexity.
