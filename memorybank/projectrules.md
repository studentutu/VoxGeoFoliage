...
- `VegetationRendererFeature` - URP integration: stores the shared `VegetationClassify.compute` reference and diagnostics toggle in `VegetationFoliageFeatureSettings`, schedules the indirect vegetation main-light shadow/depth/color passes, appends camera-visible vegetation casters into the URP main-light shadow atlas through a render-graph unsafe pass, defaults the shadow path to `TreeL3`/impostor casters unless expanded shadow promotion is explicitly enabled, and must fault-disable instead of throwing from URP callbacks; it must not perform synchronous GPU readback or rely on one mutable prepared resident frame across camera and explicit-frustum consumers
- `VegetationIndirectRenderer` - indirect rendering authority: draw-slot submission metadata, shared GPU-resident packed instance/args/slot-start buffers, explicit per-draw global slot/buffer binding, conservative uploaded-batch snapshots for diagnostics, per-slot draw metadata logging for crash isolation, and `RenderMeshIndirect` submission. Submission filtering must not depend on hot-path synchronous CPU readback.
