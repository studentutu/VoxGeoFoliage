...
30. Runtime visible-instance memory must stay hard-bounded. `VegetationRuntimeContainer.maxVisibleInstanceCapacity` is the current packed-instance budget per container; overflow must clamp safely instead of reallocating scene-scale per-slot slices, and pipeline setup must rebuild when that budget changes.
31. Container capacity is not a global scene coordinator. Multiple active containers each own independent packed instance buffers, indirect args, and GPU pipeline state. Splitting a forest across containers is allowed and is the intended chunking workaround when one container would overflow, but total memory and visible capacity then scale with the number of visible containers.
32. Current overflow policy is not production-safe for dense forests. Slot-order packing must not remain the final arbiter of what survives. Any work on runtime capacity or visibility prioritization must follow `DetailedDocs/urgentRedesign.md`: guaranteed tree presence first, then nearest-tree and nearest-branch promotion buckets.
33. Runtime diagnostics ownership is renderer-feature scoped, not container scoped. `VegetationFoliageFeatureSettings.EnableDiagnostics` affects every active `VegetationRuntimeContainer` rendered by that feature.
...
- `VegetationRuntimeContainer` - runtime orchestration hub: validates and consumes its explicit serialized `VegetationTreeAuthoring` list, freezes the runtime registry, maintains runtime tree indices, prepares one GPU-resident per-camera frame, rebuilds the GPU pipeline when `maxVisibleInstanceCapacity` changes, and binds the latest indirect draw resources into `VegetationIndirectRenderer`; nested child containers own their own descendants, current contract is explicit snapshot registration rather than live transform/content sync, and its visible-instance budget is per-container rather than global-scene-wide
