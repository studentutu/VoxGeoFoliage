...
- `VegetationRuntimeManager` - runtime orchestrator: gathers `VegetationTreeAuthoring` instances, freezes the Phase D runtime registry, maintains runtime tree indices, runs the CPU reference mirror, and exposes the immediate GPU parity hook when the compute environment is actually usable
